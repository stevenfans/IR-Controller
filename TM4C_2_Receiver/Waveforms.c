...
void sawtoothWave(int i){
		GPIO_PORTB_DATA_R = i ; 	
}

void triangleWave(int i){
	if (i < 128) GPIO_PORTB_DATA_R = i; 
	else GPIO_PORTB_DATA_R = (256 - i); 
}

void squareWave (int i){
	if (i < 128) GPIO_PORTB_DATA_R = 0x00; 
	else GPIO_PORTB_DATA_R = 0xFF; 
}
void sineWave(int i){	
